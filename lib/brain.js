...
function buildSystem(tenant, ctx = {}) {
  ...
  const sgNow = new Date().toLocaleString('en-SG', { timeZone: 'Asia/Singapore', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true });
  L.push(`Right now in Singapore it is ${sgNow}. Use this to resolve relative dates like "tomorrow" or "this Saturday" into real calendar dates. Singapore is UTC+8.`);
  L.push('');
  ...
  L.push('When the customer wants to book, start, sign up, or come down, call request_booking. Try to pin down a concrete day AND time in one gentle question (for example "would Saturday at 10am work?") so a real calendar invite can be sent, and pass it as start_iso. If they stay vague, book with preferred_time only and the team will confirm the exact slot. Never invent a time the customer did not agree to.');
...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'The customer wants to book, start, sign up, or come in. If they have agreed a CONCRETE day and time, pass start_iso so a real calendar invite is created. If the timing is still vague, pass only preferred_time and the team will confirm the slot.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        start_iso: { type: 'string', description: 'the agreed start datetime in ISO 8601 with the Singapore offset, e.g. 2026-07-04T10:00:00+08:00. Only when a concrete day AND time were agreed. Never guess.' },
        duration_minutes: { type: 'number', description: 'expected duration, default 60' },
        notes: { type: 'string' },
      },
      required: [],
    },
