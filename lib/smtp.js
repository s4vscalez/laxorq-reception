...
// opts = { to, subject, body, ics? } — when `ics` is set, the mail is sent as
// multipart/alternative with a text/calendar METHOD:REQUEST part, which Gmail,
// Outlook and Apple Mail render as a real calendar invite with an RSVP box.
function sendEmail(cfg, { to, subject, body, ics }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let msg;
    if (ics) {
      const b = 'lxq' + Date.now().toString(36) + 'boundary';
      msg = [
